...
import { BEP20Token } from "../typechain";
...
describe("BEP20 defaults", () => {

    let token: BEP20Token;
    let accounts: any;
    let owner: any;
    ...
    before(async () => {
        ...
        const tokenFactory = await ethers.getContractFactory("BEP20Token", owner);
        token = (await tokenFactory.deploy(thirtyBillion)) as BEP20Token;
        await token.deployed();

        expect(await token.name()).to.eq("BEP20 Token");
        expect(await token.symbol()).to.eq("$BEP");
